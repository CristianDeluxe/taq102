...
int main(void) {
    ...
    struct canvas a = { .px = calloc(w * h, 4), .w = w, .h = h }; statusbar_paint(&a, &st, &bitmap);
    assert(lit(&a) > 100);
    struct statusbar_style inter = bitmap; inter.font_path = "br2-external/package/taq102-fonts/fonts/Inter-SemiBold.ttf";

    struct canvas b = { .px = calloc(w * h, 4), .w = w, .h = h }; statusbar_paint(&b, &st, &inter);
    ...
    struct canvas fallback = { .px = calloc(w * h, 4), .w = w, .h = h };
