...
The chronological record of what was measured and why, in the order it was
learned. The reference material -- how to build, flash and get a console --
is in the top-level `README.md`; this file is the reasoning behind it.
...
The lesson is about controls, not about the review. Once the panel had refused every
...
**The flicker I see is not yet measured.** Both kernels, both
...
static rescue screen I see no flicker; with `glcube` he does. The
...
I corrected which hub was which.)
...
I see the flicker again. Underneath, the system was fine -- up 11
...
What only a hand at the tablet can check: a finger on the glass (interrupt
count on line 130 of `/proc/interrupts` must climb), the button, and the
flicker.
...
as MS-DOS. The first draft of the design came back with ten blocking issues, all settled
in the second, and the work went out as bounded runs, one task each.
...
Six open doubts were put to a second opinion and came back settled.
...
the loop, panic button included, for up to 300 ms. The frames before and after are in `docs/evidence/2026-09-12-desk-review/`.
