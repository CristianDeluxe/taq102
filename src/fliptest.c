...
int main(int argc, char **argv) {
    // fliptest [color-hex-a] [color-hex-b]: default identical amber; give two
    // colours to see whether alternating content shows as flicker.
    ...
    for (int i = 1;; i ^= 1) {
        flip_done = 0;
        if (drmModePageFlip(fd, crtc_id, fb[i], DRM_MODE_PAGE_FLIP_EVENT, NULL)) { perror("flip"); return 1; }
        while (!flip_done && poll(&pfd, 1, 100) > 0) drmHandleEvent(fd, &evctx);
        if (++flips % 550 == 0) { printf("fliptest: %lu flips\n", flips); fflush(stdout); }
    }
}
